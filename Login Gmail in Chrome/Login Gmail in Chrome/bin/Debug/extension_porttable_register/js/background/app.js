...
const _CLEAR_COOKIE = () => {
	chrome.cookies.getAll({domain: ".youtube.com"}, function(cookies) {
	    for(var i=0; i < cookies.length;i++) {
	        chrome.cookies.remove({ url: "https://www.youtube.com", name: cookies[i].name });
	        chrome.cookies.remove({ url: "https://accounts.youtube.com", name: cookies[i].name });
	        chrome.cookies.remove({ url: "https://accounts.google.com", name: cookies[i].name });
	    };
	});

	chrome.cookies.getAll({domain: ".google.com"}, function(cookies) {
	    for(var i=0; i < cookies.length;i++) {
	        chrome.cookies.remove({ url: "https://www.google.com", name: cookies[i].name });
	        chrome.cookies.remove({ url: "https://accounts.youtube.com", name: cookies[i].name });
	        chrome.cookies.remove({ url: "https://accounts.google.com", name: cookies[i].name });
	    };
	});
